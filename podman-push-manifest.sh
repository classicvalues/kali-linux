...
if [ -n "${CI_JOB_TOKEN:-}" ]; then
    podman manifest push "$CI_REGISTRY_IMAGE/$IMAGE:$VERSION"
    podman manifest push "$CI_REGISTRY_IMAGE/$IMAGE":latest
fi

if [ -n "${DOCKER_HUB_ACCESS_TOKEN:-}" ]; then
    podman manifest push docker.io/"$DOCKER_HUB_ORGANIZATION/$IMAGE":latest
fi
